...
func (lg *Logger) LoggerResetOutput() {
...
func (lg *Logger) logFileTemplate(useStackTrace bool) *logrus.Entry {
	entry := lg.LogFile.WithField("service", lg.service).WithField("version", lg.version)
	if useStackTrace {
...
func (lg *Logger) logConsoleTemplate(useStackTrace bool) *logrus.Entry {
	entry := lg.LogConsole.WithField("service", lg.service).WithField("version", lg.version)
	if useStackTrace {
		stackTrace := getCallerFilePath()
		entry = entry.WithField("stackTrace", stackTrace)
	}
